...
if ($_SESSION['role'] == 'Eleve'){
	...
	$demande = $bdd->prepare("SELECT * FROM aide WHERE id = :id AND eleve = :eleve");
	$demande ->execute([':id' => $_GET['id'], ':eleve' => $_SESSION['pseudo']]);
	$nb = $demande->rowCount();

	$demandeLienExpert = $bdd->prepare("SELECT * FROM aide, utilisateurs WHERE aide.expert = utilisateurs.pseudo AND aide.id = :id AND eleve = :eleve");
	$demandeLienExpert ->execute([':id' => $_GET['id'], ':eleve' => $_SESSION['pseudo']]);
	$nbLienExpert = $demandeLienExpert->rowCount();

	if ($nb > 0){
		if ($nbLienExpert > 0){
			$getEmail = $demandeLienExpert->fetch();
